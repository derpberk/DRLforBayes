...
class DuelingVisualNetwork(nn.Module):
	...
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		...
		adv_hid = self.drop_layer(F.relu(self.advantage_hidden_layer(feature)))
		val_hid = self.drop_layer(F.relu(self.value_hidden_layer(feature)))
...
class NoisyDuelingVisualNetwork(nn.Module):
	...
	def forward(self, x: torch.Tensor) -> torch.Tensor:
		"""Forward method implementation."""
		feature = self.drop_layer(self.feature_layer(x))
		feature = self.drop_layer(F.relu(self.common_layer_1(feature)))
		feature = self.drop_layer(F.relu(self.common_layer_2(feature)))
		feature = self.drop_layer(F.relu(self.common_layer_3(feature)))
